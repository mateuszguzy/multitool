...
class TestRecon:
    def test_recon_object_created_successfully(self, test_recon_whole_phase):
        assert isinstance(test_recon_whole_phase, Recon)

    def test_recon_object_runs_directory_bruteforce_module_successfully(
        self, mocker, test_recon_whole_phase
    ):
        mocker.patch.object(DirectoryBruteforce, "run")
        test_recon_whole_phase.run()
